...
export default {
    async fetch(request, env) {
		...
		if (path.startsWith(`/file/`)) {
			...
			let bucketName=env['BUCKET_NAME'];
		
		    // Ensure the second part of the path matches the bucket name
		    if (parts[2] === bucketName) {
		        // Remove '/file/bucketName' from the path
		        path = path.replace(`/file/${bucketName}/`, '/');
		    } else {
		        return new Response(null, {
		            status: 404,
		            statusText: "Bucket name is not found."
		        });
		    }
		}
